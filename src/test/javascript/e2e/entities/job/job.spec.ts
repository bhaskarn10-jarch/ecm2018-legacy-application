...
describe('Job e2e test', () => {
  ...
  it('should load Jobs', async () => {
    ...
    expect(await jobComponentsPage.getTitle()).to.eq('ecm2018LegacyApp.job.home.title');
  ...
  it('should load create Job page', async () => {
    ...
    expect(await jobUpdatePage.getPageTitle()).to.eq('ecm2018LegacyApp.job.home.createOrEditLabel');
  ...
  it('should delete last Job', async () => {
    ...
    expect(await jobDeleteDialog.getDialogTitle()).to.eq('ecm2018LegacyApp.job.delete.question');
